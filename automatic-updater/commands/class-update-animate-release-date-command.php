<?php
/**
 * Command for updating Animate release date field specifically
 */
class Update_Animate_Release_Date_Command extends Base_Command {
	...
	/**
	 * ターゲットサイト関連の発売日フィールドを更新する（専用コマンド）
	 *
	 * ## OPTIONS
	 *
	 * [--force]
	 ...
	 *
	 *     # ドライランモードでテスト実行
	 *     $ wp wp-auto update-animate-release-date --dry-run
	 *
	 *     # 詳細なログ出力
	 *     $ wp wp-auto update-animate-release-date --verbose
	 *
	 *     # 強制実行（確認なし）
	 *     $ wp wp-auto update-animate-release-date --force
	 *
	 * @when after_wp_load
	 */
	public function __invoke( $args, $assoc_args ) {
		...
		// Show execution plan
		if ( ! $this->show_execution_plan( 'ターゲットサイト発売日フィールド更新', $options ) ) {
			return;
		}
		...
		foreach ( $records as $record ) {
			...
			// 発売日フィールドのチェック（通常はindex 0）
			if ( isset( $data['cg_feed_custom_id'] ) && is_array( $data['cg_feed_custom_id'] ) ) {
				if ( isset( $data['cg_feed_custom_id'][0] ) && $data['cg_feed_custom_id'][0] === self::OLD_RELEASE_XPATH ) {
					$needs_update = true;
					if ( $options['verbose'] ) {
						WP_CLI::log( "ID: {$record->camp_id} - 古い発売日XPathが見つかりました" );
					}
				}
			}
		...
		if ( empty( $update_needed_records ) ) {
			WP_CLI::success( '更新が必要なレコードが見つかりませんでした。全ての発売日フィールドは正しく設定されています。' );
			return;
		}
		...
		// 確認を求める
		if ( ! $options['force'] ) {
			WP_CLI::confirm( sprintf( 'これらの %d 件のレコードの発売日フィールドを更新しますか？', count( $update_needed_records ) ) );
		}
		...
		// Process records
		$results = $this->process_release_date_records( $update_needed_records, $table_name, $options );
	 ...
	 */
	private function process_release_date_records( $records, $table_name, $options ) {
		$results = array(
			'success_count' => 0,
			'fail_count'    => 0,
			'skipped_count' => 0,
		...
		// Show progress bar
		$progress = \WP_CLI\Utils\make_progress_bar( '発売日フィールドを更新中', count( $records ) );

		foreach ( $records as $record ) {
			$progress->tick();

			$record_result = $this->update_single_release_date( $record, $table_name, $options );
	 ...
	 * @return array Result
	 */
	private function update_single_release_date( $record, $table_name, $options ) {
		global $wpdb;
		...
		// 発売日フィールドの更新（index 0のみ）
		if ( isset( $data['cg_feed_custom_id'][0] ) && $data['cg_feed_custom_id'][0] === self::OLD_RELEASE_XPATH ) {
			if ( $options['verbose'] ) {
				WP_CLI::log( "ID: {$record->camp_id} - 古い発売日XPathを更新 (index 0)" );
			}

			// XPathをclassセレクタに変更
			$data['cg_feed_custom_id'][0] = self::NEW_RELEASE_CLASS;
			...
			// 配列が不足している場合は拡張
			while ( count( $data['cg_custom_selector'] ) <= 0 ) {
				$data['cg_custom_selector'][] = 'class';
			}
			$data['cg_custom_selector'][0] = 'class';

			// cg_feed_css_size の調整（発売日は single が適切）
			if ( ! isset( $data['cg_feed_css_size'] ) ) {
				$data['cg_feed_css_size'] = array();
			}
			while ( count( $data['cg_feed_css_size'] ) <= 0 ) {
				$data['cg_feed_css_size'][] = 'single';
			}
			$data['cg_feed_css_size'][0] = 'single';

			// cg_feed_css_wrap の調整（発売日は inner が適切）
			if ( ! isset( $data['cg_feed_css_wrap'] ) ) {
				$data['cg_feed_css_wrap'] = array();
			}
			while ( count( $data['cg_feed_css_wrap'] ) <= 0 ) {
				$data['cg_feed_css_wrap'][] = 'inner';
			}
			$data['cg_feed_css_wrap'][0] = 'inner';
		...
		if ( ! $updated ) {
			if ( $options['verbose'] ) {
				WP_CLI::log( "ID: {$record->camp_id} - 発売日フィールドは既に正しく設定されています。" );
			}
			$result['skipped'] = true;
			return $result;
		}
		...
		if ( $update_result === false ) {
			...
			if ( $options['verbose'] ) {
				WP_CLI::success( "ID: {$record->camp_id} - 発売日フィールドの更新が完了しました。" );
			}
			$result['success'] = true;
		}
